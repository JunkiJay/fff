            ...
            <div class="grid lg:flex items-center justify-between gap-4">
                ...
                <div class="content__providers hidden md:flex col-span-1 relative justify-end"
                    ...
                    <Button variant="default" @click.native="onClickProvider" :isDisabled="false"
                        ...
                        {{ providers.current?.title === "Все провайдеры" ? "Все провайдеры" : (providers.current?.title || "Все провайдеры") }}
                        <IconArrow />
                    </Button>

                    <div class="provider absolute top-[calc(100%+10px)] w-[260px] py-2 pr-1 border-2 border-[#FFFFFF25] bg-[#1F1B29] rounded-[8px] z-20"
                        v-if="providers.status" v-on-clickaway="closeProviders">
                        <ul class="provider__list">
                            <li class="flex items-center" v-for="(item, index) in providers.items"
                                :key="index" @click="selectProvider(item)">
                                <Button :isDisabled="false" variant="provider" class="flex items-center gap-2 px-2 h-full !rounded-[5px] w-auto">
...
export default {
    ...
    async mounted() {
        ...
        // Убедиться что current установлен
        if (!this.providers.current || !this.providers.current.title) {
            if (this.providers.items.length > 0) {
                this.providers.current = this.providers.items[0];
            } else {
        ...
        if (providerFromQuery) {
            ...
            } else if (this.providers.items[0]) {
                this.selectProvider(this.providers.items[0]);
            }
        } else {
            if (this.providers.items[0]) {
                this.selectProvider(this.providers.items[0]);
            }
        }
    ...
    methods: {
        ...
        async getSlots() {
            try {
                // Проверка что current провайдер существует
                if (!this.providers.current || !this.providers.current.title) {
                    console.error('Провайдер не выбран');
                    this.loading = false;
                    return;
                }
                ...
                if (isAllProviders) {
                    ...
                    // Обработка данных от обоих провайдеров с проверкой структуры
                    const mobuleData = mobuleResponse.data?.data || [];
                    const b2bData = b2bResponse.data?.data || [];
                    
                    const mobuleSlots = Array.isArray(mobuleData) ? mobuleData.map(slot => ({
                        ...slot,
                        isB2B: false
                    })) : [];

                    const b2bSlots = Array.isArray(b2bData) ? b2bData.map(slot => ({
                        ...slot,
                        isB2B: true
                    })) : [];
                    ...
                    this.last_page = Math.max(
                        mobuleResponse.data?.last_page || 1,
                        b2bResponse.data?.last_page || 1
                    );
                } else {
                    ...
                    slotsData = Array.isArray(responseData) ? responseData.map(slot => ({
                        ...slot,
                        isB2B: slot.provider_url === "b2b"
                    })) : [];

                    this.last_page = parseInt(response.data?.last_page || 1);
                }
        ...
        async loadProviders() {
            try {
                const [mobuleRes, b2bRes] = await Promise.all([
                    axios.get(`/api/mobule/providers`).catch(() => ({ data: [] })),
                    axios.get(`/api/b2b/providers`).catch(() => ({ data: [] })),
                ]);

                // Проверка что данные - массивы
                const mobuleData = Array.isArray(mobuleRes.data) ? mobuleRes.data : [];
                const b2bData = Array.isArray(b2bRes.data) ? b2bRes.data : [];

                const providersData = [
                    ...mobuleData.map((provider) => ({
                        ...provider,
                        source: "mobule",
                    })),
                    ...b2bData.map((provider) => ({
                        ...provider,
                        source: "b2b",
                    })),
                ];
                ...
                // Устанавливаем хотя бы "Все провайдеры" если произошла ошибка
                if (this.providers.items.length === 0) {
                    this.providers.items = [{
                        title: "Все провайдеры",
                        icon: prod1Img,
                ...
                // Убедиться что current установлен даже при ошибке
                if (!this.providers.current || !this.providers.current.title) {
                    this.providers.current = this.providers.items[0] || {
                        title: "Все провайдеры",
                        icon: prod1Img,
        ...
        selectProvider(selectedProvider) {
            if (!selectedProvider || !selectedProvider.title) {
                console.error('Некорректный провайдер:', selectedProvider);
                return;
            }
...
.provider {
    ...
    &__list {
        ...
        gap: 2px; /* Минимальный пробел между элементами */
        ...
        li {
            ...
            /* Компактная кнопка по содержимому без левого отступа */
            .button {
                margin: 0;
                padding: 0 2px 0 0; /* Минимальный правый отступ */
                min-height: 32px;
                height: 32px;
