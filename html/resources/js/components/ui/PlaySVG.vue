<template>
    <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64" fill="none">
        <path
            d="M42.25 10.7825C49.4255 14.9253 53.0133 16.9966 54.9788 19.1318C61.6737 26.4045 61.6737 37.5957 54.9788 44.8684C53.0133 47.0036 49.4255 49.0749 42.25 53.2177C35.0745 57.3605 31.4867 59.4319 28.6549 60.0665C19.0091 62.2282 9.31715 56.6326 6.3663 47.1982C5.49999 44.4284 5.5 40.2857 5.5 32.0001C5.5 23.7145 5.5 19.5718 6.3663 16.802C9.31715 7.36765 19.0091 1.77201 28.6549 3.93369C31.4867 4.56831 35.0745 6.6397 42.25 10.7825Z"
            fill="url(#paint0_linear_1708_3562)"
        />
        <mask id="mask0_1708_3562" style="mask-type: alpha" maskUnits="userSpaceOnUse" x="5" y="3" width="55" height="58">
            <path
                d="M42.25 10.7825C49.4255 14.9253 53.0133 16.9966 54.9788 19.1318C61.6737 26.4045 61.6737 37.5957 54.9788 44.8684C53.0133 47.0036 49.4255 49.0749 42.25 53.2177C35.0745 57.3605 31.4867 59.4319 28.6549 60.0665C19.0091 62.2282 9.31715 56.6326 6.3663 47.1982C5.49999 44.4284 5.5 40.2857 5.5 32.0001C5.5 23.7145 5.5 19.5718 6.3663 16.802C9.31715 7.36765 19.0091 1.77201 28.6549 3.93369C31.4867 4.56831 35.0745 6.6397 42.25 10.7825Z"
                fill="url(#paint1_linear_1708_3562)"
            />
        </mask>
        <g mask="url(#mask0_1708_3562)">
            <path
                d="M23.4053 41.8506L37.5 29.1304L40.7934 29.1304L55.831 44.168L40.7769 59.2222L23.4053 41.8506Z"
                fill="url(#paint2_linear_1708_3562)"
                fill-opacity="0.65"
            />
        </g>
        <path
            d="M40 28.5365C42.6667 30.0761 42.6667 33.9251 40 35.4647L28 42.3929C25.3333 43.9325 22 42.008 22 38.9288L22 25.0724C22 21.9932 25.3333 20.0687 28 21.6083L40 28.5365Z"
            fill="url(#paint3_linear_1708_3562)"
        />
        <defs>
            <linearGradient id="paint0_linear_1708_3562" x1="13" y1="3" x2="40.5" y2="56" gradientUnits="userSpaceOnUse">
                <stop stop-color="#1d4e7b" />
                <stop offset="1" stop-color="#1d4e7b" />
            </linearGradient>
            <linearGradient id="paint1_linear_1708_3562" x1="10" y1="9.5" x2="48.5" y2="49" gradientUnits="userSpaceOnUse">
                <stop stop-color="#FF5C5C" />
                <stop offset="1" stop-color="#1d4e7b" />
            </linearGradient>
            <linearGradient id="paint2_linear_1708_3562" x1="36.5" y1="38.0002" x2="45.4165" y2="45.8129" gradientUnits="userSpaceOnUse">
                <stop stop-color="#1d4e7b" />
                <stop offset="1" stop-color="#1d4e7b" stop-opacity="0" />
            </linearGradient>
            <linearGradient id="paint3_linear_1708_3562" x1="22.5" y1="22.5005" x2="54.5" y2="53.5005" gradientUnits="userSpaceOnUse">
                <stop offset="0.104437" stop-color="white" />
                <stop offset="1" stop-color="#1d4e7b" />
            </linearGradient>
        </defs>
    </svg>
</template>
