...
use App\Services\SevenxpayPayment\SevenxpayPaymentService;
use Exception;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;
use Illuminate\Support\Facades\Validator;

class SevenxpayPaymentController extends Controller
{
    private $paymentService;

    public function __construct(SevenxpayPaymentService $paymentService)
    {
    ...
    public function createPayment(Request $request)
    {
        try {
            ...
            $paymentData = [
                'client_txn_id' => (string)$payment->id,
                'payer_ip' => (string)$request->ip(),
                'payer_id' => (string)$user->id,
                'sum' => $amount,
                "currency_id" => 1,
                'comment' => 'super hero',
                'callback_url' => config('app.url') . '/xpay/webhook',
                'success_url' => config('app.url') . '/pay',
                'fail_url' => config('app.url') . '/pay'
            ];
