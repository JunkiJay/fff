...
class PSPayPaymentProvider extends PaymentProvider
{
    public function __construct(
        PaymentProviderConfig $config,
        public readonly PSPayApiClient $apiClient
    ) {
    ...
    public function pay(Payment $payment): PaymentShowSBPResult|PaymentErrorResult
    {
        try {
            ...
            try {
                ...
                Log::error('Failed to decode PSPay error response', ['exception' => $decodeException]);
            }
        ...
        if ($result->status === 'waiting') {
            Log::debug('PSPay response', ['response' => $result]);
