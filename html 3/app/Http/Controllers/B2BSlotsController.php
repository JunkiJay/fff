...
class B2BSlotsController extends Controller
{
    ...
    public function loadSlot(Request $request)
    {
        $user = $request->user();
        if (!$user) {
            return response()->json(['error' => true, 'message' => 'Пользователь не найден'], 404);
        }
        ...
        if (!$cur) {
            if ($user->is_youtuber) {
                $link = "https://int.apichannel.cloud/gamesbycode/$slot->gm_bk_id.gamecode?operator_id=" . ($user->is_youtuber ? '40114' : '40115') . "&language=ru&user_id={$user->id}&auth_token={$user->auth_token}&currency=RUB&desktop_interface=1&home_url=https://stimule.win/slots";
            } else {
                if ($psum < 100) {
                    $link = "https://int.apichannel.cloud/gamesbycode/$slot->gm_bk_id.gamecode?operator_id=40115&language=ru&user_id={$user->id}&auth_token={$user->auth_token}&currency=RUBB&desktop_interface=1&home_url=https://stimule.win/slots";
                } else {
                    $link = "https://int.apichannel.cloud/gamesbycode/$slot->gm_bk_id.gamecode?operator_id=" . ($user->is_youtuber ? '40114' : '40115') . "&language=ru&user_id={$user->id}&auth_token={$user->auth_token}&currency=RUB&desktop_interface=1&home_url=https://stimule.win/slots";
                }
            }
