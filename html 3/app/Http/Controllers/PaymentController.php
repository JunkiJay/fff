...
class PaymentController extends Controller
{
    ...
    public function create1plat(Request $request)
    {
        ...
        $shopId = 11; // Ваш shop_id
        $secret = 'UB3W8GV378RH23UFIWEH52WEDFF'; // Ваш секретный ключ
        ...
        // URL API
        $apiUrl = 'https://1plat.cash/api/merchant/order/create/by-api';
    ...
    public function createSpinPay(Request $request)
    {
        ...
        $params = [
            ...
            "redirectSuccessUrl" => "https://stimule1.win",
            "redirectFailUrl" => "https://stimule1.win",
            "callbackUrl" => "https://stimule1.win/callback/spinpay68fsdd2",
            "orderNumber" => strval($order->id),
            "customer" => [
                "email" => "user" . $user->id . "@gmail.com",
                "ip" => $this->getIp(),
        ...
        if (isset($data["success"]) && $data["success"] && !empty($data["processingUrl"][0]["trader"])) {
            ...
            $errorMessage = $data["error_message"] ?? "Не удалось получить URL";
    ...
    {
        $sign = md5(
            $this->config->kassa_id . ':' . $request->AMOUNT . ':' . $this->config->kassa_secret2 . ':' . $request->MERCHANT_ORDER_ID
        );
    ...
    public function createParadise(Request $request)
    {
        ...
        // Данные для аутентификации
        $shop_id = 8;
        $secret = 'prod_FdxJk1vWwn6whLOc4sQ3GF0g'; // Ваш секретный ключ
        ...
        // URL API
        $apiUrl = 'https://api.p2p-paradise.info/payments';
    ...
    public function createGtx(Request $request)
    {
        ...
        return [
            'error' => true,
            'message' => 'Попробуйте попытку через 15 минут или используйте другой метод'
        ]; //'Попробуйте попытку через 15 минут или используйте другой метод'
    }
    ...
    public function callbackCryptobot(Request $request)
    {
        ...
        if ($request['payload']['status'] == 'expired') {
            $payment->status = 2;
            $payment->save();
            return ['ok'];
        }

        if ($amount > 0 && $payment->status != 1 && $request['payload']['status'] == 'paid') {
            if (!is_null($user->referral_use)) {
                $this->setReferralProfit($user->id, $payment->sum);
            }
