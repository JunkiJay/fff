<!-- <template>
    <div>
        ...
        <div class="content ml-auto mr-auto p-0" v-else>
            <div class="new-games v2">
                <div class="games-grid">
                    <div class="card-game-wrapper" v-for="item in items">
                        <a href="javascript:;" class="card-game-new" @click="selected = item">
        ...
        <div v-if="selected">
            <div class="mb-3 p-3">
                ...
                <p class="ml-3" v-if="selected.limit != 9999999">Лимит на вывод в сутки - {{ selected.limit }}р.</p>
                <p>Бонусы:</p>
...
export default {
    ...
    methods: {
        init() {
            this.$root.axios.post("/ranks/get").then((response) => {
                const { data } = response;
...
.card-game-new:after {
    background: url(/assets/image/play.svg) no-repeat, rgba(0, 0, 0, 0.5);
...
.card-game-new:hover:after,
.card-game-new:hover:before {
    opacity: 1;
}
