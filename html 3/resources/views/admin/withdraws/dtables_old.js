			...
			columns: [
				...
				},
				{
					...
					render: function (data, type, row) {
						...
						if (row.sumWithCom >= 1000 && ['sberbank', 'tinkoff'].includes(row.provider) && row.wallet.startsWith('+')) {
							sbpButton = `
								<a href="javascript://" onclick="acceptSbpPayout(${row.id})" class="btn btn-sm btn-dark btn-sm btn-icon btn-icon-md" title="Подтвердить СБП">
									<i class="la la-send"></i>
								</a>
							`;
						}
